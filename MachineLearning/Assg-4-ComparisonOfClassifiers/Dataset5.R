...
#d <- read.csv("http://archive.ics.uci.edu/ml/machine-learning-databases/ionosphere/ionosphere.data", header=FALSE, sep = ",")
...
for(i in 1:10) 
{
  ...
  #--------------DECISION TREE----------------
  #Model-
  dTTrainModel <- rpart(as.factor(V35) ~ ., data = trainingData, parms=list(split="information"), method ="class", minsplit=2)
  ...
  #Model
  sVMTrainModel <- svm(as.factor(V35) ~ ., data =trainingData,method="C-classification")
  #Predict
  ...
  nBTrainModel <- naiveBayes(as.factor(V35) ~ ., data = trainingData,laplace=5)
  #prediction
  ...
  #---------------KNN-----------------
  #Model for kNN
  f1<-factor(trainingData$V35)
  #convert to numeric
  trainingData[,as.integer(35)] <- as.numeric(trainingData[,as.integer(35)])
  testData[,as.integer(35)] <- as.numeric(testData[,as.integer(35)])
  #Model for kNN
  kNNMmodel <- knn(trainingData, testData, f1, k = 9, l = 0 , prob = FALSE, use.all = TRUE)
  kNNTable <- table(testData$V35, kNNMmodel)
  ...
  lRTrainModel <- glm(as.factor(V35)~V13+V15+V23+V21+V19+V17, data = trainingData, family = "binomial") 
  ...
  sapply(d, sd)
  #take variables for formula with sapply value > 0.6
  nNTrainModel <- neuralnet(as.integer(V35) ~ V13+V15+V23+V21+V19+V17, trainingData, hidden = 8, lifesign = "minimal", 
                          linear.output = FALSE, threshold = 0.1)
  
  temp_test <- subset(testData, select = c("V13", "V15", "V23","V21","V19","V17"))
  nNCompute <- compute(nNTrainModel, temp_test)
  results <- data.frame(actual = testData$V35, prediction = nNCompute$net.result) 
  ...
  #----------------BAGGING-------------
  #Model 
  bagTrainModel <- ada(V35 ~ ., data = trainingData,control = rpart.control(maxdepth=1))
  #predict
  bagPredict <-predict(bagTrainModel, testData)
  #accuracy
  baggingTable <- table(testData$V35, bagPredict)
  ...
  #----------------RANDOM FOREST-------------
  #Model 
  rFTrainmodel <- ctree(as.factor(V35) ~ ., data=trainingData)
  #predict
  rFPredict <- predict(rFTrainmodel,testData)
  #accuracy
  rFTable <- table(testData$V35, rFPredict)
  ...
  boostingTrainModel <- ada(V35 ~ ., data = trainingData, iter=20, nu=1, type="discrete")
  boostingPredict=predict(boostingTrainModel,testData)
  
  boostingTable <- table(testData$V35, boostingPredict)
